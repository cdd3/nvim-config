...
local plugin_specs = {
    ...
    --      1)  functions for installing, updating, and removing tree-sitter
    --          parsers;
    --      2)  a collection of queries for enabling tree-sitter features built
    --          into Neovim for these languages;
    --      3)  a staging ground for treesitter-based features considered for
    ...
    ----------------------------------------------------------------------------
    -- Note: 2020-03-16 - This plugin has caused me a LOT of pain on windows
    -- and is therefore disabled currently.  I'm not sure of it's usefulness at
    -- this point and consequently if it's worth the effor to to 'figure it out'
    ----------------------------------------------------------------------------
    --  {
    ...
    --  document with as few keystrokes as possible. It does so by annotating
    --  text in your buffer with hints, short string sequences for which each
    --  character represents a key to type to jump to the annotated text. Most of
    --  the time, those sequences’ lengths will be between 1 to 3 characters,
    --  making every jump target in your document reachable in a few keystrokes.
    --  aka "Super fast buffer jump"
    ----------------------------------------------------------------------------
    -- {
    --   "smoka7/hop.nvim",
    --   keys = { "f" },
    ...
    ----------------------------------------------------------------------------
    --  nvim-hlslens helps you better glance at matched information, seamlessly
    --  jump between matched instances.
    --  i.e. "Show match number and index for searching"
    ----------------------------------------------------------------------------
    {
    ...
    ----------------------------------------------------------------------------
    -- A lot of people have been mapping jk or kj or kk to ESC in order to
    -- escape insert mode quickly. However, when we press the first key in these
    -- mappings, Vim/Nvim will wait for timeoutlen milliseconds before writing
    -- this char to buffer. The apparent lag caused by this behaviour is annoying.

    -- Better-escape.vim is created to help Vim/Nvim users escape insert mode
    -- quickly using their customized key combinations, without experiencing
    -- the lag.
    ----------------------------------------------------------------------------
    {
    ...
    ----------------------------------------------------------------------------
    -- Since tmux is only available on Linux and Mac, we only enable these
    -- plugins for Linux and Mac
    ...
    ----------------------------------------------------------------------------
    -- Vim plugin by Tim Pope that provides various commands and mappings to aid
    -- in editing, debugging, and exploring Vimscript
    ----------------------------------------------------------------------------
    ...
    -- enable you to run shell commands in the background and read the output in
    -- the quickfix window in realtime
    ----------------------------------------------------------------------------
    -- {
    ...
    ----------------------------------------------------------------------------
    -- TODO: 2026-03-16 revisit this.  github page says:
    -- 'Vim syntax for TOML. As of Neovim 0.6 and Vim 8.2.3519, these runtime
    -- files are included in the main (Neo)Vim distribution.'
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    -- CopilotChat.nvim brings GitHub Copilot Chat capabilities directly into
    -- Neovim with a focus on transparency and user control.
    ----------------------------------------------------------------------------
    -- {
    ...
    ----------------------------------------------------------------------------
    -- dependency for above CopilotChat.nvim
    ----------------------------------------------------------------------------
    -- {
    ...
    ----------------------------------------------------------------------------
    -- Text editing in Neovim with immediate visual feedback: see the effects of
    -- any command on your buffer in real-time. Preview macros, the :norm
    -- command, and more!
    ----------------------------------------------------------------------------
    -- {
    ...
    -- This makes code actions both discoverable and efficient, as code actions
    -- can be available even when there are no visible diagnostics (warning,
    -- information, hints etc.).
    ...
    ----------------------------------------------------------------------------
    -- A polished, IDE-like, highly-customizable winbar for Neovim with
    -- drop-down menus and multiple backends.
